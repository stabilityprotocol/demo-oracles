"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateWeather = void 0;
const inversify_1 = require("inversify");
const Weather_1 = require("../DataProviders/Weather");
const Oracle_1 = require("../Contracts/Oracle");
let UpdateWeather = class UpdateWeather {
    constructor(weatherDataProvider, oracleContract) {
        this.weatherDataProvider = weatherDataProvider;
        this.oracleContract = oracleContract;
    }
    async execute() {
        const weatherData = await this.weatherDataProvider.getCurrentWeather();
        for (let data of weatherData) {
            await this.oracleContract.setValue(data.city, data.temperature.toString());
        }
    }
};
UpdateWeather = __decorate([
    (0, inversify_1.injectable)(),
    __param(0, (0, inversify_1.inject)(Weather_1.WeatherDataProvider)),
    __param(1, (0, inversify_1.inject)(Oracle_1.OracleContract)),
    __metadata("design:paramtypes", [Weather_1.WeatherDataProvider,
        Oracle_1.OracleContract])
], UpdateWeather);
exports.UpdateWeather = UpdateWeather;
//# sourceMappingURL=UpdateWeather.js.map